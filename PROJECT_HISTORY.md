...
- **Review wizard**: Optional "Photo" section added to Step 2 (after Steps, before Save) — shows AI-generated image preview with "Replace" overlay for recipes; dashed "Add a photo" zone for songs/stories/wisdom/poems. Photo uploads after save using the returned token; failure is non-blocking (memory saved regardless)
...
- **Mic bug fix**: `startRecording()` wrapped in try/catch; `pickMimeType()` used for cross-browser mimeType detection (Safari fix)
...
- `new MediaRecorder(stream, { mimeType: 'audio/webm' })` throws `DOMException` on Safari silently as unhandled promise rejection; wrap entire `startRecording` in try/catch
